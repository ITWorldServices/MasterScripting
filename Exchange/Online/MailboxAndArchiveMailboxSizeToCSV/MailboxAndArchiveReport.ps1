...
@{n="Archive Item Count";e={(Get-MailboxStatistics -archive $_.identity).ItemCount}} | export-csv c:\ArchiveMailboxSizeReport_$((Get-Date -format MMM-dd` hh-mm` tt).ToString()).csv -nti