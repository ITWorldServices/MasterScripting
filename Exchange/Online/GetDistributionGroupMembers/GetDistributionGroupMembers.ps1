...
Function main()
{
 ...
 #Set output file 
 $ExportCSV="c:\Impact\DistributionGroup-DetailedMembersReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" #Detailed report
 $ExportSummaryCSV="C:\Impact\DistributionGroup-SummaryReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" #Summary report

 #Get a list of RecipientTypeDetail
 $RecipientTypeArray=Get-Content -Path "c:\impact\RecipientTypeDetails.txt" -ErrorAction Stop
 $Result=""  
 $Results=@()
