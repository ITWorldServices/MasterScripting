...
<#
.SYNOPSIS
This script performs OneDrive preprovisioning for a list of users and generates reports on the results.

.DESCRIPTION
The script executes a command (likely for OneDrive preprovisioning) for each user email provided. It then:
1. Checks for any errors during the provisioning process.
2. Records the status (Success or Failed) for each user.
3. Outputs a summary of successful and failed provisioning attempts.
4. Exports all results to a CSV file.
5. Exports failed provisioning attempts to a separate CSV file if any exist.
6. Disconnects from the SharePoint Online service.

.NOTES
- The script assumes a connection to SharePoint Online has been established prior to execution.
- It uses a predefined $command variable, which should contain the OneDrive preprovisioning command.
- The script expects a list of user emails to be processed.
- Results are stored in the $results array as custom objects.
- Two CSV files are generated: one for all results and another for failed attempts (if any).
- The script uses color-coded console output for better readability.

.OUTPUTS
- Console output with color-coded status messages.
- CSV file with all provisioning results.
- CSV file with failed provisioning attempts (if any).

.EXAMPLE
.\OneDrivePreprovisioning.ps1
...
if ($tenantType -eq "Standard") {
    $tenantUrl = "https://maksplastics-admin.sharepoint.com"
...
$exportPath = "C:\temp\MAKsPlastics-OneDriveProvisioningResults.csv"
