...
$ResetPasswords = Get-YesNoInput "Do you want to connect to Azure AD and reset passwords for non-admin users?"
...
if ($ResetPasswords) {
    ...
    function Generate-StrongPassword {
        ...
        foreach ($char in $ExcludeChars.ToCharArray()) {
            $upper = $upper -replace [regex]::Escape($char), ""
            $lower = $lower -replace [regex]::Escape($char), ""
            $numbers = $numbers -replace [regex]::Escape($char), ""
            $special = $special -replace [regex]::Escape($char), ""
        }

        $chars = @()
        if ($IncludeUppercase -and $upper.Length -gt 0) { $chars += $upper.ToCharArray() }
        if ($IncludeLowercase -and $lower.Length -gt 0) { $chars += $lower.ToCharArray() }
        if ($IncludeNumbers -and $numbers.Length -gt 0) { $chars += $numbers.ToCharArray() }
        if ($IncludeSpecialChars -and $special.Length -gt 0) { $chars += $special.ToCharArray() }

        if ($chars.Count -eq 0) {
            throw "No characters available for password generation after exclusions."
        }

        $password = @()
        if ($IncludeUppercase -and $upper.Length -gt 0) { $password += Get-Random -InputObject $upper.ToCharArray() }
        if ($IncludeLowercase -and $lower.Length -gt 0) { $password += Get-Random -InputObject $lower.ToCharArray() }
        if ($IncludeNumbers -and $numbers.Length -gt 0) { $password += Get-Random -InputObject $numbers.ToCharArray() }
        if ($IncludeSpecialChars -and $special.Length -gt 0) { $password += Get-Random -InputObject $special.ToCharArray() }
    ...
    # Prompt for characters to exclude
    $ExcludeChars = Read-Host "Enter any characters you want to EXCLUDE from passwords (leave blank for none)"

    # Prompt for force password change at next login
    $ForceChangeAtNextLogin = Get-YesNoInput "Force users to change password at next login?"

    $resetPasswords = @()

    foreach ($user in $usersToReset) {
        $newPassword = Generate-StrongPassword -Length $PasswordLength `
        ...
        try {
            Set-AzureADUserPassword -ObjectId $user.ObjectId `
                ...
                -ForceChangePasswordNextLogin:$ForceChangeAtNextLogin
            ...
            Write-Host "Error resetting password for user $($user.UserPrincipalName): $($Error[0].Exception.Message)" -ForegroundColor Red
        }
    }
